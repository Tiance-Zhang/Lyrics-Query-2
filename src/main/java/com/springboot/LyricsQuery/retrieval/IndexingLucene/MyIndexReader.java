package com.springboot.LyricsQuery.retrieval.IndexingLucene;
...
public class MyIndexReader {
	...
	public MyIndexReader( String dataType ) throws IOException {
		if (dataType.equals("trectext")) {
			directory = FSDirectory.open(Paths.get(com.springboot.LyricsQuery.retrieval.Classes.Path.IndexTextDir));
		} else {
			directory = FSDirectory.open(Paths.get(com.springboot.LyricsQuery.retrieval.Classes.Path.IndexWebDir));
		}
	...
	/**
	 * Get the data type of collection
	 */
	...
	public int getDocid( String docno ) throws IOException {
		...
		Query query = new TermQuery(new Term("SONG", docno));
	...
	public String getDocno( int docid ) throws IOException {
		...
		return (doc==null)?null:doc.get("SONG");
	}
	...
	public int[][] getPostingList( String token ) throws IOException {
		...
		for (ScoreDoc score : scoreDoc){
			int id = score.doc;
			int freq=0;
			vector = ireader.getTermVector(id, "LYRICS");
	...
	public int docLength( int docid ) throws IOException {
		int doc_length = 0;
		Terms vector = ireader.getTermVector( docid, "LYRICS" );
		if (vector == null) return 0;
		if (docid == 19949) System.out.println(vector.toString());
